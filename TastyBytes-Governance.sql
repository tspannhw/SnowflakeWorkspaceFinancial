...
USE WAREHOUSE catcloud_tastybyteszerotosnowflake_dev_wh;
...
CREATE OR REPLACE ROLE catcloud_tastybyteszerotosnowflake_test_role
...
GRANT ALL ON WAREHOUSE catcloud_tastybyteszerotosnowflake_dev_wh TO ROLE sysadmin;


-- next we will grant only OPERATE and USAGE privileges to our Test Role
GRANT OPERATE, USAGE ON WAREHOUSE catcloud_tastybyteszerotosnowflake_dev_wh TO ROLE catcloud_tastybyteszerotosnowflake_test_role;
...
GRANT USAGE ON DATABASE catcloud_tastybyteszerotosnowflake_prod TO ROLE catcloud_tastybyteszerotosnowflake_test_role;

GRANT USAGE ON ALL SCHEMAS IN DATABASE catcloud_tastybyteszerotosnowflake_prod TO ROLE catcloud_tastybyteszerotosnowflake_test_role;
...
GRANT SELECT ON ALL TABLES IN SCHEMA catcloud_tastybyteszerotosnowflake_prod.raw_customer TO ROLE catcloud_tastybyteszerotosnowflake_test_role;

GRANT SELECT ON ALL TABLES IN SCHEMA catcloud_tastybyteszerotosnowflake_prod.raw_pos TO ROLE catcloud_tastybyteszerotosnowflake_test_role;

GRANT SELECT ON ALL VIEWS IN SCHEMA catcloud_tastybyteszerotosnowflake_prod.analytics TO ROLE catcloud_tastybyteszerotosnowflake_test_role;
...
GRANT ROLE catcloud_tastybyteszerotosnowflake_test_role TO USER identifier($my_user_var);
...
USE ROLE catcloud_tastybyteszerotosnowflake_test_role;
USE WAREHOUSE catcloud_tastybyteszerotosnowflake_dev_wh;
USE DATABASE catcloud_tastybyteszerotosnowflake_prod;
...
FROM TABLE(information_schema.tag_references_all_columns
    ('catcloud_tastybyteszerotosnowflake_prod.raw_customer.customer_loyalty','table'));
...
INSERT INTO governance.row_policy_map
    VALUES ('CATCLOUD_TASTYBYTESZEROTOSNOWFLAKE_TEST_ROLE','Tokyo'); 
...
WHERE object_name ILIKE 'catcloud_tastybyteszerotosnowflake_prod.raw_%'
...
WHERE 1=1
    AND object_name ILIKE 'catcloud_tastybyteszerotosnowflake_prod.raw_%'
...
(
    ref_entity_name => 'catcloud_tastybyteszerotosnowflake_prod.raw_customer.customer_loyalty',
...
WHERE table_database = 'CATCLOUD_TASTYBYTESZEROTOSNOWFLAKE_PROD'
...
-- drop Test Role
DROP ROLE IF EXISTS catcloud_tastybyteszerotosnowflake_test_role;
