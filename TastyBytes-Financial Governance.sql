...
USE ROLE catcloud_tastybyteszerotosnowflake_admin;
USE WAREHOUSE catcloud_tastybyteszerotosnowflake_de_wh;
USE DATABASE catcloud_tastybyteszerotosnowflake_prod;

-- assign Query Tag to Session 
ALTER SESSION SET query_tag = '{"origin":"sf_sit-is","name":"tb_zts","version":{"major":1, "minor":1},"attributes":{"is_quickstart":0, "source":"tastybytes", "vignette": "cost_management"}}';
...
SHOW WAREHOUSES LIKE '%catcloud_tastybyteszerotosnowflake%';
...
CREATE OR REPLACE WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh WITH
...
CREATE OR REPLACE RESOURCE MONITOR catcloud_tastybyteszerotosnowflake_test_rm
WITH
...
ALTER WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh 
    SET RESOURCE_MONITOR = catcloud_tastybyteszerotosnowflake_test_rm;

SHOW PARAMETERS IN WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh;
...
SHOW PARAMETERS LIKE 'STATEMENT%' IN WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh;
...
ALTER WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh SET TAG cost_center = 'DEVELOPMENT_TEAM';
...
SELECT 
    ...
    level,
    object_name
FROM TABLE(information_schema.tag_references('catcloud_tastybyteszerotosnowflake_test_wh', 'warehouse'));
...
USE ROLE catcloud_tastybyteszerotosnowflake_admin;
USE WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh;
...
ALTER WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh SET warehouse_size = 'XLarge';


-- using a Show Warehouses command, we can confirm the current size of our Test Warehouse
SHOW WAREHOUSES LIKE 'catcloud_tastybyteszerotosnowflake_test_wh';
...
ALTER WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh SET warehouse_size = 'XSmall';
...
ALTER WAREHOUSE catcloud_tastybyteszerotosnowflake_test_wh SUSPEND;
...
-- drop Test Warehouse
DROP WAREHOUSE IF EXISTS catcloud_tastybyteszerotosnowflake_test_wh;

-- drop Cost Center Tag
DROP TAG IF EXISTS cost_center;

-- drop Resource Monitor
DROP RESOURCE MONITOR IF EXISTS catcloud_tastybyteszerotosnowflake_test_rm;
